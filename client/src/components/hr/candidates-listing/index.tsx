...
@observer
export default class CandidatesListingView extends React.Component<Props, {}> {
    ...
    return (
      <div id="base-view">
        <section className="listing-title">
          <p className="size36 semibold">Candidates</p>
          <p className="size36 semibold mob">Candidates</p>
        </section>
        {this.hasDocs ?
        <section className="listing-content ga">
          ...
          <ul className="listing">
            {this.docs.map((res: CandidatesRequest.CandidatesListingModel) =>
              <li className="user" key={res._id} onClick={() => this.redirectToCandidatesPage(res)}>
                ...
                <p className="size14 medium">{res.department.name}</p>
                <p className="size14 medium">{res.email}</p>
                <p className="size14 medium">{<DateFormatter date={res.date}/>}</p>
                <p className="size14 medium">{res.comment? res.comment: '-'}</p>
          ...
          :
          <p className='no-data'>There is no candidates yet.</p>
        }
      </div>
    )
  }
